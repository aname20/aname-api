...
export class EventsService {
    ...
    private async validatePermission(dependentId: string, userId: string) {
        ...
        if (!hasAccess) {
            throw new ForbiddenException('You do not have permission to access events for this dependent.');
        }
    }
    ...
    async create(createEventDto: CreateEventDto, userId: string) {
        ...
        const { doctorId, ...rest } = createEventDto;
    ...
    async update(id: number, updateEventDto: UpdateEventDto, userId: string) {
        ...
        if (updateEventDto.dependentId && updateEventDto.dependentId !== existingEvent.dependentId) {
            await this.validatePermission(updateEventDto.dependentId, userId);
        }

        const { doctorId, ...rest } = updateEventDto;
